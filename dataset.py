...
def get_df(kernel_type, data_dir, train_step):
    ...
    if train_step == 0:
        ...
        df_train_nonlandmark = pd.read_csv(os.path.join(data_dir, 'train_nonlandmark.csv'))
    else:
    ...
    df_train['landmark_id'] = 0
    df_train_xtra['filepath'] = df_train_xtra['id'].apply(lambda x: os.path.join(data_dir, 'train_xtra', x[0], x[0], x[1], x[2], f'{x}.jpg'))
    df_train_xtra['landmark_id'] = 0
    df_train_nonlandmark['filepath'] = df_train_nonlandmark['id'].apply(lambda x: os.path.join(data_dir, 'train_nonlandmark', x[0], x[0], x[1], x[2], f'{x}.jpg'))
    df_train_nonlandmark['landmark_id'] = 1
    df_train = pd.concat([df_train, df_train_xtra, df_train_nonlandmark], axis=0).reset_index(drop=True)
